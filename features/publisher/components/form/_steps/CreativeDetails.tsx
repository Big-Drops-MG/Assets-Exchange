        ...
        <div>
                                    ...
                                    <p className="text-sm text-green-600">
                                        ...
                                        formData.creativeType === "email" ? (
                                            (() => {
                                                const fromLines = uploadedFiles[0].fromLines || formData.fromLines || "";
                                                const subjectLines = uploadedFiles[0].subjectLines || formData.subjectLines || "";
                                                const fromCount = fromLines ? fromLines.split("\n").filter((line) => line.trim()).length : 0;
                                                const subjectCount = subjectLines ? subjectLines.split("\n").filter((line) => line.trim()).length : 0;
                                                return fromCount > 0 || subjectCount > 0 ? (
                                                    <>
                                                        {" • "}
                                                        {fromCount} from lines • {subjectCount} subject lines
                                                    </>
                                                ) : null;
                                            })()
                                        ) : null}
          ...
          <SingleCreativeViewModal
            ...
            onFileNameChange={(fileId, newFileName) => {
              ...
              if (selectedCreative.id === fileId) {
                const updatedCreative = { ...selectedCreative, name: newFileName };
