...
<body>
  ...
  const constrainPosition = useCallback((x: number, y: number, zoom: number, containerWidth: number, containerHeight: number, imgWidth: number, imgHeight: number) => {
    if (zoom <= 1) {
      return { x: 0, y: 0 };
    }
  ...
  const handleZoomIn = useCallback(() => {
    setImageZoom((prev) => {
      ...
      if (newZoom > 1 && imageDimensions.width > 0 && imageDimensions.height > 0) {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (isDragging && imageZoom > 1 && imageDimensions.width > 0 && imageDimensions.height > 0) {
