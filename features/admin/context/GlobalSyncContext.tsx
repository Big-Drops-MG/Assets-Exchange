...
interface SyncState {
  ...
  startSync: (
    type: "everflow_sync" | "everflow_sync_advertisers"
  ) => Promise<void>;
  cancelSync: (
    type: "everflow_sync" | "everflow_sync_advertisers"
  ) => Promise<void>;
...
export function GlobalSyncProvider({
    ...
    const checkActiveJobs = async () => {
      try {
        const offersRes = await fetch(
          "/api/admin/everflow/active-job?type=everflow_sync"
        );
        if (offersRes.ok) {
      ...
      try {
        const advRes = await fetch(
          "/api/admin/everflow/active-job?type=everflow_advertiser_sync"
        );
        if (advRes.ok) {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await fetch(
          `/api/admin/everflow/sync-status/${offersSync.jobId}`
        );
        if (!res.ok) return;
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await fetch(
          `/api/admin/everflow/sync-status/${advertisersSync.jobId}`
        );
        if (!res.ok) return;
        const data = await res.json();
  ...
  const startSync = async (
    syncType: "everflow_sync" | "everflow_sync_advertisers"
  ) => {
    const endpoint =
      syncType === "everflow_sync"
        ? "/api/admin/everflow/sync"
        : "/api/admin/everflow/advertisers/sync";

    const setSyncState =
      syncType === "everflow_sync" ? setOffersSync : setAdvertisersSync;
  ...
  const cancelSync = async (
    ...
    const syncState =
      syncType === "everflow_sync" ? offersSync : advertisersSync;
    if (!syncState.jobId) return;

    try {
      await fetch(`/api/admin/everflow/cancel/${syncState.jobId}`, {
        method: "POST",
      });
      const setSyncState =
        syncType === "everflow_sync" ? setOffersSync : setAdvertisersSync;
