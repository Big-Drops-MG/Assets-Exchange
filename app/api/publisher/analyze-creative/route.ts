...
${creativeText}`;
        ...
        const copyTxtFile = new File([txtContent], "generated_copy.txt", {
            type: "text/plain",
        });
        ...
        if (pythonServiceUrl) {
            try {
                ...
                if (pythonResponse.ok) {
                    pythonAnalysis = (await pythonResponse.json()) as Record<string, unknown>;
                } else {
                    console.warn(
        ...
        const message =
            error instanceof Error ? error.message : "Failed to run analysis pipeline";
