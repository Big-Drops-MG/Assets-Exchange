...
export async function POST(
  ...
  // Allow both admin and administrator roles for job replay
  if (
    !session ||
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Replay limit exceeded. Please wait 5 minutes before retrying.",
        },
        { status: 429 }
      );
    }
    ...
    logger.app.info(
      `Job replayed manually - originalJobId: ${jobId}, newJobId: ${newJob.id}, userId: ${session.user.id}`
    );
